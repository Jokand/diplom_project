...
public class TestGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            if (hero.available_equipment.size()==0){
            ...
            if (category_clothes) {
                ...
                try {
                    weapon weapon_avatar = hero.weapon;
                    hero.weapon = (weapon) hero.available_equipment.get(inventory_vote);
                    weapon choose = (weapon) hero.available_equipment.get(inventory_vote);
                    hero.available_equipment.remove(inventory_vote);
                    hero.available_equipment.add(weapon_avatar);
                    System.out.println("Вы надели на себя " + choose.name + ". Теперь ваш модификатор урона равен " + hero.weapon.bonus_damage);
                } catch (Exception ex) {
                    System.out.println("Неккоректный ввод");
                }
            }
